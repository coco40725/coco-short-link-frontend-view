...
watch([currentExpireDate, currentExpireTime], () => {
    if (!currentExpireDate.value || !currentExpireTime.value) {
        ...
        console.log(currentEditLinkInfo.value.expirationDate)
    }
...
<template lang="pug">
.disable-link-container(v-if="isShortLinkInfoLoaded")
    .header
        .title QR code
        .title 原始網址
        .title 短網址
        ...
        .title 過期時間
        .title 功能
    .content(v-for="urlInfo in disabledShortLink" :key="urlInfo.id")
        .item.qr-code-img
        ...
        .item.actions
            button.delete-button(@click="enableLink(urlInfo)") 啟動
